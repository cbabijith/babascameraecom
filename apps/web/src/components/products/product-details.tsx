...
export default function ProductDetails({ productId }: ProductDetailsProps) {
  ...
  useEffect(() => {
    ...
    // Scroll helpers deliberately read current refs to avoid recreating listeners.
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const run = async () => {
      try {
        ...
        setSelectedVariantId(
          productData.variants?.find((variant) => variant.stock > 0)?.id ??
            productData.variants?.[0]?.id ??
            null,
        );

        setYouMayLoading(false);
        setYouMayAlso(productData.relatedProducts ?? []);
        setYouMayTotal(productData.relatedProducts?.length ?? 0);
      } catch (err) {
  ...
  const salePrice = product.price.salePrice + Number(selectedVariant?.additionalPrice ?? 0);
  const actualPrice = product.price.actualPrice;
  const discount =
    actualPrice !== salePrice
      ? Math.round(
          (1 - salePrice / actualPrice) * 100
        )
      : 0;
  ...
  const variantRows: { label: string; value: React.ReactNode }[] =
    (product.variants ?? []).map((variant) => ({
      label: variant.name,
      value: `${variant.value}${Number(variant.additionalPrice) ? ` (+${formatPrice(Number(variant.additionalPrice))})` : ""}`,
    }));
  ...
  const renderVariantSelector = () => {
    ...
    return (
      <div className="space-y-2">
        <p className="text-sm font-medium text-gray-900">Choose an option</p>
          ...
          {variants.map((variant: ProductVariantRow) => {
            ...
            return (
              ...
              >
                ...
                {Number(variant.additionalPrice) !== 0 && ` (${Number(variant.additionalPrice) > 0 ? "+" : ""}${formatPrice(Number(variant.additionalPrice))})`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-4">
            ...
            <div className="lg:hidden">
              ...
              <div className="mt-4">{renderVariantSelector()}</div>

              {isInStock ? (
                (selectedVariant?.stock ?? product.quantity) < 5 && (
                  <Badge
                    variant="secondary"
                    className="mt-2 border bg-green-100 text-green-800 border-green-300"
                  >
                    {`Only ${selectedVariant?.stock ?? product.quantity} left in stock!`}
                  </Badge>
                )
              ) : (
          ...
          <div className="space-y-6">
            ...
            {(product.reviewCount ?? 0) > 0 && (
              <div className="flex items-center gap-2 text-sm text-gray-700">
                ...
                <span>
                  from {product.reviewCount} approved review
